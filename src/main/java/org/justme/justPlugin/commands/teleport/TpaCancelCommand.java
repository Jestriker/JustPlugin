...
public class TpaCancelCommand implements TabExecutor {
    ...
    public boolean onCommand(@NotNull CommandSender sender, @NotNull Command cmd, @NotNull String label, @NotNull String[] args) {
        ...
        if (tm.isWaitingToTeleport(player.getUniqueId())) {
            ...
            player.sendMessage(plugin.getMessageManager().success("teleport.tpacancel.tp-cancelled"));
            return true;
        }
        // Cancel outgoing request
        TeleportManager.TpaRequest req = tm.getOutgoingRequest(player.getUniqueId());
        if (req == null) {
            player.sendMessage(plugin.getMessageManager().error("teleport.tpacancel.no-pending"));
            return true;
        }
        tm.cancelOutgoingRequest(player.getUniqueId());
        player.sendMessage(plugin.getMessageManager().success("teleport.tpacancel.request-cancelled"));
        Player target = Bukkit.getPlayer(req.target);
        if (target != null) {
            target.sendMessage(plugin.getMessageManager().warning("teleport.tpacancel.notify-target", "{player}", player.getName()));
        }
        return true;
    }
