...
/**
 * MySQL storage provider - uses HikariCP for connection pooling.
 */
public class MySQLStorageProvider implements StorageProvider {
    ...
    public MySQLStorageProvider(JustPlugin plugin, String host, int port, String database,
    ...
    public void init() throws Exception {
        ...
        createTables();
        plugin.getLogger().info("[Database] MySQL storage provider initialized (" + host + ":" + port + "/" + database + ").");
    }
    ...
    public void shutdown() {
        ...
        plugin.getLogger().info("[Database] MySQL storage provider shut down.");
    }
    ...
    public void saveBan(String key, Map<String, Object> data) {
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "REPLACE INTO bans (id, data) VALUES (?, ?)")) {
            ps.setString(1, key);
    ...
    public void deleteBan(String key) {
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM bans WHERE id = ?")) {
            ps.setString(1, key);
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    public void saveTeam(String name, Map<String, Object> data) {
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "REPLACE INTO teams (name, data) VALUES (?, ?)")) {
            ps.setString(1, name);
            ps.setString(2, gson.toJson(data));
            ps.executeUpdate();
        } catch (SQLException e) {
    ...
    public void deleteTeam(String name) {
        try (Connection conn = getConnection();
             PreparedStatement ps = conn.prepareStatement("DELETE FROM teams WHERE name = ?")) {
            ps.setString(1, name);
            ps.executeUpdate();
        } catch (SQLException e) {
